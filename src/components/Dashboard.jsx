  ...
  const getMaintenanceAlerts = () => {
    ...
    return maintenanceEntries
      ...
      .map(entry => {
        ...
        return { ...entry, status, categoryId: category.id, categoryColor: category.color, remainingKm: Math.max(0, entry.nextDueODO - currentOdometer) };
      })
  ...
  const getUpcomingMaintenance = () => {
    ...
    return maintenanceEntries
      ...
      .map(entry => {
        ...
        const category = getMaintenanceCategory(entry.type);
        return { ...entry, categoryId: category.id, categoryColor: category.color, kmRemaining, daysRemaining, projectedDate };
      })
    ...
    <div className="flex flex-col h-[calc(100vh-180px)] overflow-hidden">
      <PageWrapper className="flex-1 flex flex-col min-h-0 space-y-6">
        <div className="flex-shrink-0">
          <div className="mb-4">
            <h1 className="text-xl font-semibold text-slate-900 dark:text-white tracking-tight">{t('overview')}</h1>
          </div>
          ...
          <section className="grid grid-cols-2 gap-3 mb-6">
            <MetricCard className="flex flex-col justify-between min-h-[120px] p-4">
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-slate-400">Avg Km/L</span>
              </div>
            ...
            <MetricCard variant="secondary" className="flex flex-col justify-between min-h-[120px] p-4">
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-slate-400">L/100km</span>
              </div>
            ...
            <MetricCard className="flex flex-col justify-between min-h-[120px] p-4">
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-slate-400">{t('total_spent')}</span>
              </div>
              <div className="flex items-baseline gap-1">
                ...
                <span className="text-[10px] font-medium text-slate-500">EGP</span>
              </div>
            </MetricCard>
            ...
            <MetricCard variant="secondary" className="flex flex-col justify-between min-h-[120px] p-4">
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-slate-400">{t('history')}</span>
              </div>
              ...
              {maintenanceAlerts.length > 0 && (
                 <Link to="/maintenance" className="w-full flex items-center justify-between p-3 bg-red-500/10 dark:bg-red-500/20 border border-red-500/20 rounded-2xl">
                    <div className="flex items-center gap-2">
                       ...
                       <span className="text-xs font-bold text-red-600 dark:text-red-400">{maintenanceAlerts.length} {t('overdue')}!</span>
                    </div>
                    <ChevronRight className={cn("w-4 h-4 text-red-400", isRtl && "rotate-180")} />
                 </Link>
              )}
              {upcomingMaintenance.length > 0 && (
                <button onClick={() => setPredictedModalOpen(true)} className="w-full flex items-center justify-between p-3 bg-blue-500/10 dark:bg-blue-500/20 border border-blue-500/20 rounded-2xl">
                  <div className="flex items-center gap-2">
                    <Route className="w-4 h-4 text-blue-500" />
                    <span className="text-xs font-bold text-blue-600 dark:text-blue-400">{t('due_soon')}: {t(upcomingMaintenance[0].categoryId)}</span>
                  </div>
                  <ChevronRight className={cn("w-4 h-4 text-blue-400", isRtl && "rotate-180")} />
                </button>
              )}
            </div>
          )}
        ...
        <section className="flex-1 flex flex-col min-h-0">
          <div className="flex items-center justify-between mb-3 px-1 flex-shrink-0">
            <h2 className="text-sm font-bold text-slate-900 dark:text-white tracking-tight">{t('history')}</h2>
            <Link to="/history" className="text-xs font-medium text-emerald-500 dark:text-emerald-400">{t('overview')}</Link>
          </div>
        ...
        <Modal isOpen={predictedModalOpen} onClose={() => setPredictedModalOpen(false)} title={t('due_soon')}>
                ...
                <div key={item.id} className="rounded-2xl p-4 bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20">
                   <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                         <p className="text-sm font-bold text-blue-700 dark:text-blue-400">{t(item.categoryId)}</p>
                         <p className="text-xs text-slate-500 mt-0.5">{item.kmRemaining.toLocaleString()} km left</p>
                      </div>
                      <div className="text-end text-xs font-bold text-blue-600 dark:text-blue-400">{format(item.projectedDate, 'MMM d')}</div>
                   </div>
                </div>
             ))}
          </div>
